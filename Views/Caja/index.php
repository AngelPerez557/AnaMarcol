                                ...
                                <tr class="producto-item"
                                    ...
                                    <td class="ps-3">
                                        <div class="d-flex align-items-center gap-2">
                                            <div style="width:44px;height:44px;flex-shrink:0;border-radius:6px;background-image:url('<?= $producto->getImageUrl() ?>');background-size:contain;background-position:center;background-repeat:no-repeat;background-color:#fdf8f8;border:1px solid #dee2e6;"></div>
                                            <div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-layer-group me-2" style="color:#de777d;"></i>Seleccionar variante
                </h5>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (e) {
        ...
        if (tieneVariantes) {
            ...
            if (stock <= 0) {
                Swal.fire({ icon:'warning', title:'Sin stock', text:`"${nombre}" no tiene stock disponible.`, confirmButtonColor:'#de777d' });
                return;
            }
    ...
    document.getElementById('btnCobrar').addEventListener('click', function () {
        if (!carrito.length) {
            Swal.fire({ icon:'warning', title:'Carrito vacío', text:'Agrega productos antes de cobrar.', confirmButtonColor:'#de777d' });
            return;
        }
        ...
        if (metodo === 'Efectivo' && recibido < total) {
            Swal.fire({ icon:'warning', title:'Monto insuficiente', text:'El monto recibido es menor al total.', confirmButtonColor:'#de777d' });
            return;
        }
        ...
        document.getElementById('bodyConfirmar').innerHTML = `
            ...
            <div class="border-top pt-2">
                ...
                <div class="d-flex justify-content-between fs-5">
                    <span>Total:</span><strong style="color:#de777d;">L. ${total.toFixed(2)}</strong>
                </div>
                ${metodo === 'Efectivo' ? `
                <div class="d-flex justify-content-between">
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                Swal.fire({
                    ...
                    html:`<div class="fs-4 fw-bold" style="color:#de777d;">Total: L. ${parseFloat(data.total).toFixed(2)}</div>
                ...
                Swal.fire({ icon:'error', title:'Error', text: data.message || 'No se pudo procesar.', confirmButtonColor:'#de777d' });
            }
        })
        .catch(() => {
            Swal.fire({ icon:'error', title:'Error de conexión', text:'No se pudo conectar.', confirmButtonColor:'#de777d' });
        })
        .finally(() => {
            this.disabled = false;
